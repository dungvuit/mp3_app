...
RSpec.describe CommentsController, type: :controller do
  context 'If user login as guest' do
    ...
    it '#Delete Comment' do
      ...
      expect do
        delete :destroy, params: { id: @comment.id, song_id: @song.id, format: :js }
      end.to change(Comment, :count).by(-1)
    end

    it "#Edit Comment" do
      @comment = Comment.create(song_id: @song.id, user_id: @user.id, content: "abc")
      patch :update, params: { id: @comment.id, song_id: @song.id, content: "aaa", format: :js }
      @comment.reload
